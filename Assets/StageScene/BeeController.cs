...
public class BeeController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            audioSource.PlayOneShot(needle);                     // 강화 원거리 공격 오디오 재생
    ...
    {
        if (collision.gameObject.CompareTag("DebufGround"))
        {
            this.maxWalkSpeed *= 0.5f;                   // 땅과 충돌 시 속도 감소
        }
    }
    ...
    void OnCollisionExit2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("DebufGround"))
        {
            this.maxWalkSpeed = 4.0f;                   // 땅과 충돌 시 속도 감소
        }
    }

    void ShootStinger(float positionx, float positiony, float distance)
    {
        audioSource.PlayOneShot(needle);                     // 원거리 공격 오디오 재생
    ...
    {
        audioSource.PlayOneShot(ulti);                     // 살충기 공격 오디오 재생
    ...
    {
        audioSource.PlayOneShot(push);                     // 근거리 공격 오디오 재생
    ...
    void PerformMeleeAttack(bool isEnhanced = false)
    {
        ...
        float damage = isEnhanced ? 0.1f : 0.05f; // 강화 공격 시 더 큰 데미지
